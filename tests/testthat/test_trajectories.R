test_that("traj2lines works", {
  ...
  traj_ls <- lapply(df_ls, traj2lines, clon = "lon", clat = "lat")
  ...
  expect_true(all(vapply(df_ls, function(x) {
    length(traj2lines(x, clon = "lon", clat = "lat")) == nrow(x) - 1
  }, logical(1))))
...
test_that("format_traj_names works", {
  ...
  test_names <-
    c("ALF_2011_12_30_15_4419.60", "ALF_2011_04_16_16_2133.60",
      "ALF_2011_11_02_16_609.60",  "ALF_2011_01_12_16_609.60",
      "ALF_2011_01_24_16_457.20")
  expt_names <-
    c("ALF_2011_12_30_4419.6_15", "ALF_2011_04_16_2133.6_16",
...
test_that("get_trajectory_metadata works", {
  ...
  traj_df <- get_trajectory_metadata(
    files = files,
    cnames = names(TRAJECTORY.METADATA),
    ctypes = TRAJECTORY.METADATA
  )

  expect_true(all(nrow(traj_df) <= 10, ncol(traj_df) == 7))
  expect_equal(
    object = colnames(traj_df),
    expected = c("site", "year", "month", "day", "hour", "height", "file_path")
  )
